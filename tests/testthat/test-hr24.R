test_that("hr_24 works with todays date", {
  skip_on_ci()
...
test_that("hr_24 works with data from three days back", {
  skip_on_ci()
  d1 <- fetch_hr24(filedate = Sys.Date() - 3) |> dplyr::as_tibble()

  is_valid <- nrow(d1) > 1000

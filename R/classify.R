...
#' @importFrom httr GET upload_file POST accept content
...
classify_umu_ub <- function(record, type = c("mods", "wos"),
  ...
  diva_mods_url <- function(diva_pid) {
    sprintf(paste0(
      ...
      "?referenceFormat=MODS&pids=[%s]&fileName=export.xml"
    ), diva_pid)
  }

  filedata_mods <- function(pid) {
    mods <- httr::GET(diva_mods_url(pid))
    mods_file <- tempfile()
    xml2::write_xml(httr::content(mods), mods_file)
...
issues_swepub <- function(institution = "kth", year_beg = 2012L,
  ...
  res <- httr::with_config(
    ...
    httr::GET(
      sprintf(
        paste0(
          "https://bibliometri.swepub.kb.se/api/v1/",

...
genres <- 
  kthcorpus:::genre_scopus_diva() |> 
  rename(subtypeDescription = subtype)

keys <- 
  pubtypes |> left_join(genres, by = c("subtypeDescription", "prism:aggregationType")) |> 
  mutate(key = if_else(is.na(key), "other", key)) |> 
  left_join(tibble(key = kthcorpus:::kth_genre() |> map(names) |> getElement("publicationType")), by = "key") |> 
  pull(key)
...
sids_cp <-
  import |>
  filter(
    !`dc:identifier` %in% c(sids_erratum) &

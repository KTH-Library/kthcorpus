...
hrp_extra <- function(hrp) {
    ...
    left_join(research_areas |> mutate(id = as.numeric(id)), by = c("scb_topic" = "id")) |>
    rename(
      "scb_topic_swe" = "swe",
      "scb_topic_eng" = "eng",
...
check_hr_endsbeforebeg <- function() {
  # people that end their employment even before beginning it
  hrpe |> 
    filter(emp_end <= emp_beg) |> 
  select(kthid, emp_end, emp_beg, unit_abbr, lastname, firstname) |> 
  mutate(duration = emp_beg - emp_end) |> 
...
check_missing_kthid <- function() {
  ...
  hrpe |> 
    filter(is.na(kthid)) |> 
  select(kthid, username, firstname, lastname, emp_end, emp_beg, unit_abbr) |> 
...
Some employees have a date indicating employment was ended before the start date.
